...
export default class Replacer {
  ...
  static codeblock(fullMatch, group1, group2) {
    if (typeof group2 == "undefined") return "<pre><code>" + "</code></pre>";
    else return "<pre><code>" + group2 + "</code></pre>";
  }
  ...
  static referenceLink(fullMatch, group1, group2) {
    for (const i in referenceLinkArray) {
      if (referenceLinkArray[i].id == group2)
        return (
          '<a href="' +
          referenceLinkArray[i].link +
          '" title="' +
          referenceLinkArray[i].linkTitle +
          '" target="_blank">' +
          group1 +
          "</a>"
        );
  ...
  static emoji(fullMatch, group1) {
    for (const i in emojiArray) {
      if (emojiArray[i][group1] != undefined) return emojiArray[i][group1];
    }
    return fullMatch;
  }
}
